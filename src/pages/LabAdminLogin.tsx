...
export default function LabAdminLogin() {
  ...
  const expectedEmail = import.meta.env.VITE_ADMIN_EMAIL || "admin@trivatelabs.com";
  const expectedPassword = import.meta.env.VITE_ADMIN_PASSWORD || "admin123";

  const hasUrlError = new URLSearchParams(location.search).has("error");
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    navigate("/labadmin/dashboard", { replace: true });
  };
    ...
    <div className="relative flex min-h-screen items-center justify-center bg-gray-950 p-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl border border-gray-800 bg-gray-900/95 p-8 shadow-2xl">
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          {(hasUrlError || error) && (
            <p className="text-sm text-red-400">{error || "Invalid email or password"}</p>
          )}

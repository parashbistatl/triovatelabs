    ...
    const load = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          setResources((prev) => mergeResources(prev, data as ResourceItem[]));
        }
      } catch {
        // Keep fallback static content on API failure.
      }
    };
        ...
        <section className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex w-full flex-col gap-2 sm:max-w-xl sm:flex-row">
                ...
                />
                <Select value={sortBy} onValueChange={(value) => setSortBy(value as "az" | "za")}>
          ...
          {resources.length > 0 ? (
            ...
            >
              ...
              >
                <Link to="/contact" className="rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-800">
                  Request a Resource
                </Link>
                <Link to="/services" className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-800 hover:bg-gray-100">
                  View Services
                </Link>
              </div>
            </div>
